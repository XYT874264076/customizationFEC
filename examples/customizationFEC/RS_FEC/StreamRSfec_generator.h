#ifndef EXAMPLES_CUSTOMIZATION_RSFEC_STREAMRSFEC_GENERATOR_H_
#define EXAMPLES_CUSTOMIZATION_RSFEC_STREAMRSFEC_GENERATOR_H_
...
class StreamRSfecGenerator : public VideoFecGenerator {
  friend class FlexfecSender;
 public:
  StreamRSfecGenerator(const Environment& env, int red_payload_type, int rsfec_payload_type);
  ~StreamRSfecGenerator();

  FecType GetFecType() const override {
    return VideoFecGenerator::FecType::kRSFecStream;
  }
 ...
 private:
  ...
  StreamRSfecGenerator(const Environment& env, std::unique_ptr<RSForwardErrorCorrection> fec);
  ...
  const std::unique_ptr<RSForwardErrorCorrection> fec_ RTC_GUARDED_BY(race_checker_);
  RSForwardErrorCorrection::PacketList media_packets_ RTC_GUARDED_BY(race_checker_);
  std::list<int> media_packets_key_frame_note_ RTC_GUARDED_BY(race_checker_);
  std::optional<RtpPacketToSend> last_media_packet_ RTC_GUARDED_BY(race_checker_);
  std::list<RSForwardErrorCorrection::Packet*> generated_fec_packets_ RTC_GUARDED_BY(race_checker_);
  int num_protected_frames_ RTC_GUARDED_BY(race_checker_);
  int min_num_media_packets_ RTC_GUARDED_BY(race_checker_);
  int num_media_packets_insert_ RTC_GUARDED_BY(race_checker_);
  double left_rate_ RTC_GUARDED_BY(race_checker_);
  Params current_params_ RTC_GUARDED_BY(race_checker_);
  int media_contains_keyframe_ RTC_GUARDED_BY(race_checker_);
...
#endif  //EXAMPLES_CUSTOMIZATION_RSFEC_STREAMRSFEC_GENERATOR_H_
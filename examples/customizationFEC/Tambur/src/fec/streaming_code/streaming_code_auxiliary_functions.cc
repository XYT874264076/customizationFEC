...
#include "examples/customizationFEC/Tambur/src/fec/streaming_code/streaming_code_auxiliary_functions.hh"
    ...
    parity_pkt_sizes, uint16_t packet_size)
{
  ...
  for (uint16_t pos = 0; pos < parity_pkt_sizes.size(); pos++) {
    ...
    if (parity_pkt_sizes.at(pos) != packet_size) {
      assert(false && "Not all same size parities");
    }
  }
  return payloads;
}
    ...
    BlockCode & blockCode, uint16_t delay)
{
  ...
  if (total > stripe_size * frame_max_fec_stripes) {
    assert(false && "Invalid number of parity stripes for parity pkts");
  }

...
public:
  ...
  {
    assert(max_rows > 0 && max_cols > 0 && "invalid number of rows or columns to allocate");
  ...
  {
    assert(rows <= max_rows_ && cols <= max_cols_ && "cannot resize beyond originally allocated size");
...
private:
  ...
  {
    assert(row < rows_ && col < cols_ && "invalid row or column");
  }

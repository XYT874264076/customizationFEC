...
class RLModule:
    def __init__(self, model_path, auto_save=True, save_interval=10):
        self.model = A2CModel(state_dim=12, action_dim=31)  # 输入维度12，输出维度31,分别表示 I=1~31
    ...
    def predict(self, state, reward):
        ...
        # action_idx = torch.argmax(action_probs).item()  # 获取最大概率的索引（0~30）
        # action = action_idx + 1  # 映射到1~31

        # 选项2：按概率分布随机采样（探索性选择）
        action_dist = torch.distributions.Categorical(logits=action_probs)
        action_idx = action_dist.sample().item()  # 采样索引（0~30）
        action = action_idx + 1
...
if __name__ == "__main__":
    # 示例用法
    rl = RLModule("checkpoints/model_weights.pth")
    ...
    state2 = [0.6] * 12  # 模拟状态输入
    reward2 = 0.6

    state3 = [0.7] * 12  # 模拟状态输入
    reward3 = 0.7

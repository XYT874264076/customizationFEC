      ...
      uint8_t num_frames_in_video_frame)
{
  ...
  for (uint8_t index = 0; index < frame_pkts_info.size(); index++) {
    ...
    stripe_pos += static_cast<uint8_t>(data_pkts.back().payload.size() / stripe_size);
  }
  return data_pkts;
}
      ...
      uint16_t frame_size)
{
  ...
  for (uint8_t index = 0; index < num_parity_pkts; index++) {
    const FECDatagram pkt {sqn_++, true, frame_num_,
        ...
        static_cast<uint16_t>(index + data_pkts.size())), static_cast<uint16_t>(index + data_pkts.size()),

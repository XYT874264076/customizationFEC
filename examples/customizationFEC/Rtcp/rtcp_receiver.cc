...
#include "examples/customizationFEC/Rtcp/rtcp_receiver.h"
...
#include "examples/customizationFEC/Rtcp/fec_receiver_report.h"
...
struct RTCPReceiver::PacketInformation {
  ...
  std::vector<FECReportBlockData> fec_report_block_datas;
...
RTCPReceiver::RTCPReceiver(const Environment& env,
    ...
    : env_(env),
      ...
      fec_report_block_data_observer_(config.fec_report_block_data_ovserver) {
  RTC_DCHECK(owner);
}
...
RTCPReceiver::RTCPReceiver(const Environment& env,
    ...
    : env_(env),
      ...
      num_skipped_packets_(0),
      last_skipped_packets_warning_(env_.clock().CurrentTime()),
      fec_report_block_data_observer_(nullptr) {
  RTC_DCHECK(owner);
  ...
  for (const uint8_t* next_block = packet.begin();
    ...
    switch (rtcp_block.type()) {
      ...
      case rtcp::ReceiverReport::kPacketType:
        ...
        break;
      case rtcp::FecReceiverReport::kPacketType:
        valid = HandleFECReceiverReport(rtcp_block, packet_information);
        break;
...
bool RTCPReceiver::HandleReceiverReport(const CommonHeader& rtcp_block,
  ...
  // std::cout<<"Run HandleReceiverReport!"<<std::endl;
...
bool RTCPReceiver::HandleFECReceiverReport(const rtcp::CommonHeader& rtcp_block,
  PacketInformation* packet_information) {
  rtcp::FecReceiverReport receiver_report;
  if (!receiver_report.Parse(rtcp_block)) {
    return false;
  }
  ...
  packet_information->packet_type_flags |= kRtcpFECr;

  for (const FECReportBlock& report_block : receiver_report.report_blocks()) {

    HandleFECReportBlock(report_block, packet_information, remote_ssrc);

    // if (registered_ssrcs_.contains(report_block.source_ssrc())) {
    //   std::cout<<"\t\t[RTCP Packet Data]("<<this<<")"<<std::endl;
    //   std::cout<<"\t\tsource_ssrc:"<<report_block.source_ssrc()<<std::endl;
    //   std::cout<<"\t\tfraction_lost_recover:"<<(int)report_block.fraction_lost_recover()<<std::endl;
    //   std::cout<<"\t\tfraction_effective_FEC_pkt:"<<(int)report_block.fraction_effective_FEC_pkt()<<std::endl;
    //   std::cout<<"\t\trecover_pkt_num:"<<report_block.recover_pkt_num()<<std::endl;
    //   std::cout<<"\t\taverage_recover_time:"<<report_block.average_recover_time()<<std::endl;
    //   std::cout<<"\t\taverage_retransmit_time:"<<report_block.average_retransmit_time()<<std::endl;
    //   std::cout<<"\t\ttime_ahead_for_recover:"<<report_block.time_ahead_for_recover()<<std::endl;
    //   std::cout<<"\t\tretransmit_pkt_num:"<<report_block.retransmit_pkt_num()<<std::endl;
    //   std::cout<<"\t\tboth_succ_pkt_num:"<<report_block.both_pkt_num()<<std::endl;
    //   std::cout<<std::endl;

    //   FECReportBlockData* temp_block_data = &received_fec_report_blocks_[report_block.source_ssrc()];
    //   std::cout<<"\t\t[Block Data]"<<std::endl;
    //   std::cout<<"\t\tfixed_fraction_lost_recover_raw:"<<(int)temp_block_data->fixed_fraction_lost_recover_raw()<<std::endl;
    //   std::cout<<"\t\tfixed_fraction_effective_FEC_pkt_raw:"<<(int)temp_block_data->fixed_fraction_effective_FEC_pkt_raw()<<std::endl;
    //   std::cout<<"\t\trecover_pkt_num_raw:"<<temp_block_data->recover_pkt_num_raw()<<std::endl;
    //   std::cout<<"\t\taverage_recover_time:"<<temp_block_data->fixed_average_recover_time()<<std::endl;
    //   std::cout<<"\t\taverage_retransmit_time:"<<temp_block_data->fixed_average_retransmit_time()<<std::endl;
    //   std::cout<<"\t\ttime_ahead_for_recover:"<<temp_block_data->fixed_time_ahead_for_recover()<<std::endl;
    //   std::cout<<"\t\tretransmit_pkt_num_raw:"<<temp_block_data->retransmit_pkt_num_raw()<<std::endl;
    //   std::cout<<"\t\tboth_pkt_num:"<<temp_block_data->both_succ_pkt_num_raw()<<std::endl;
    //   std::cout<<"\t\tcumulative_recover_pkt:"<<temp_block_data->cumulative_recover_pkt()<<std::endl;
    //   std::cout<<"\t\tcumulative_retransmit_pkt:"<<temp_block_data->cumulative_retransmit_pkt()<<std::endl;
    //   std::cout<<"\t\tcumulative_both_succ_pkt:"<<temp_block_data->cumulative_both_succ_pkt()<<std::endl;
    //   std::cout<<std::endl;
    // }
  }
...
void RTCPReceiver::HandleFECReportBlock(const rtcp::FECReportBlock& report_block,
                                        PacketInformation* packet_information,
                                        uint32_t remote_ssrc) {
  if (!registered_ssrcs_.contains(report_block.source_ssrc())) return;

  Timestamp now = env_.clock().CurrentTime();
  FECReportBlockData* fec_report_block_data = &received_fec_report_blocks_[report_block.source_ssrc()];
  NtpTime now_ntp = env_.clock().ConvertTimestampToNtpTime(now);
  fec_report_block_data->SetFECReportBlock(remote_ssrc, report_block, Clock::NtpToUtc(now_ntp), now);
  packet_information->fec_report_block_datas.push_back(*fec_report_block_data);
}
...
void RTCPReceiver::TriggerCallbacksFromRtcpPacket(
  ...
  if (!receiver_only_) {
    ...
    if (fec_report_block_data_observer_) {
      for (const auto& fec_report_block_data : packet_information.fec_report_block_datas) {
        fec_report_block_data_observer_->OnFECReportBlockDataUpdate(fec_report_block_data);
      }
    }
  }
}

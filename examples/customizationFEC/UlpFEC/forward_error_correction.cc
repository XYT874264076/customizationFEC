...
#include "examples/customizationFEC/UlpFEC/ulpfec_header_reader_writer.h"
...
void ForwardErrorCorrection::InsertFecPacket(
  ...
  if (fec_packet->protected_packets.empty()) {
    ...
    if (received_fec_packets_.size() > max_fec_packets) {
      ...
      fec<<milliseconds_since_epoch<<","<<(**fec_packet_it).seq_num<<","<<"fail_BufferLimit"
        <<","<<","<<protect_seq_num<<",,,"<<fec_drop_time.ns()<<"\n";
  ...
  while (fec_packet_it != received_fec_packets_.end()) {
    ...
    if (packets_missing == 1) {
      ...
      if (!RecoverPacket(**fec_packet_it, recovered_packet.get())) {
        ...
        fec<<milliseconds_since_epoch<<","<<(**fec_packet_it).seq_num<<","<<"fail_RecoverFail"
          <<","<<","<<protect_seq_num<<",,,"<<fec_drop_time.ns()<<"\n";
      ...
      fec<<milliseconds_since_epoch<<","<<(**fec_packet_it).seq_num<<","<<"success"
        <<","<<recovered_packet_ptr->seq_num<<","<<protect_seq_num<<","<<time_decode.ns()<<",,"<<fec_drop_time.ns()<<"\n";
      ...
      // TODO:Here we should further judge why this fec packet is lost!
      //Write file!
      auto now = std::chrono::system_clock::now();
      auto milliseconds_since_epoch = std::chrono::duration_cast<std::chrono::microseconds>(now.time_since_epoch()).count();
      std::ofstream fec(inputV::Params::output+"forward_error_correction.csv",std::ios::app);
      ...
      if (packets_missing == 0) {
        fec<<milliseconds_since_epoch<<","<<(**fec_packet_it).seq_num<<","<<"fail_NoLoss"
          <<","<<","<<protect_seq_num<<",,,"<<fec_drop_time.ns()<<"\n";
      }
      else {
        fec<<milliseconds_since_epoch<<","<<(**fec_packet_it).seq_num<<","<<"fail_OldFEC"
            <<","<<","<<protect_seq_num<<",,,"<<fec_drop_time.ns()<<"\n";
      }

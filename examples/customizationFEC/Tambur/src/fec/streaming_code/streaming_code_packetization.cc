...
StreamingCodePacketization::StreamingCodePacketization(uint16_t w,
    ...
    uint16_t max_pkt_size, uint16_t parity_delay) : 
    // w_(w),
    stripe_size_(stripe_size), 
    qr_to_parity_per_data_(qr_to_parity_per_data),
    // max_pkt_size_(max_pkt_size),
    parity_delay_(parity_delay)
{
  if (size_t(max_pkt_size) > Packetization::MTU)
  {
    assert(false && "Too large max_pkt_size");
  }

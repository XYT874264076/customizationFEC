...
void SendStatisticsProxy::OnFECReportBlockDataUpdate(FECReportBlockData report_block) {
  MutexLock lock(&mutex_);
  VideoSendStream::StreamStats* stats = GetStatsEntry(report_block.source_ssrc());
  if (!stats) return;

  stats->fec_report_block_data = std::move(report_block);
}

...
#include "examples/MyFECExp/frame_generator_player.h"
...
FrameGeneratorPlayer::FrameGeneratorPlayer(
    Clock* clock,
...
void FrameGeneratorPlayer::SetFakeRotation(VideoRotation rotation) {
  MutexLock lock(&lock_);
  fake_rotation_ = rotation;
}

void FrameGeneratorPlayer::SetFakeColorSpace(
    std::optional<ColorSpace> color_space) {
  MutexLock lock(&lock_);
  fake_color_space_ = color_space;
}

bool FrameGeneratorPlayer::Init() {
...
void FrameGeneratorPlayer::InsertFrame() {
  MutexLock lock(&lock_);
  if (sending_) {
    // TODO(srte): Use more advanced frame rate control to allow arbitrary
    // fractions.
    int decimation = std::round(static_cast<double>(source_fps_) / target_capture_fps_);
...
std::optional<FrameGeneratorPlayer::Resolution> FrameGeneratorPlayer::GetResolution() const {
  FrameGeneratorInterface::Resolution resolution = frame_generator_->GetResolution();
  return Resolution{.width = static_cast<int>(resolution.width),
                    .height = static_cast<int>(resolution.height)};
}

void FrameGeneratorPlayer::Start() {
  {
    MutexLock lock(&lock_);
    sending_ = true;
  }
...
void FrameGeneratorPlayer::Stop() {
  MutexLock lock(&lock_);
  sending_ = false;
}

void FrameGeneratorPlayer::ChangeResolution(size_t width, size_t height) {
  MutexLock lock(&lock_);
  frame_generator_->ChangeResolution(width, height);
}

void FrameGeneratorPlayer::ChangeFramerate(int target_framerate) {
  MutexLock lock(&lock_);
...
int FrameGeneratorPlayer::GetFrameWidth() const {
  return static_cast<int>(frame_generator_->GetResolution().width);
}

int FrameGeneratorPlayer::GetFrameHeight() const {
  return static_cast<int>(frame_generator_->GetResolution().height);
}

void FrameGeneratorPlayer::OnOutputFormatRequest(
    int width,
    int height,
    const std::optional<int>& max_fps) {
  MP4VideoPlayer::OnOutputFormatRequest(width, height, max_fps);
}

void FrameGeneratorPlayer::SetSinkWantsObserver(SinkWantsObserver* observer) {
  MutexLock lock(&lock_);
  RTC_DCHECK(!sink_wants_observer_);
  sink_wants_observer_ = observer;
}

void FrameGeneratorPlayer::AddOrUpdateSink(rtc::VideoSinkInterface<VideoFrame>* sink,
    const rtc::VideoSinkWants& wants) {
  MP4VideoPlayer::AddOrUpdateSink(sink, wants);
  {
    MutexLock lock(&lock_);
...
void FrameGeneratorPlayer::RemoveSink(
    rtc::VideoSinkInterface<VideoFrame>* sink) {
  MP4VideoPlayer::RemoveSink(sink);
  ChangeFramerate(GetSinkWants().max_framerate_fps);
}

void FrameGeneratorPlayer::ForceFrame() {
  // One-time non-repeating task,
  task_queue_->PostTask([this] { InsertFrame(); });
}

int FrameGeneratorPlayer::GetCurrentConfiguredFramerate() {
  MutexLock lock(&lock_);

...
#include "examples/customizationFEC/Rtcp/rtcp_sender.h"
...
RTCPSender::Configuration RTCPSender::Configuration::FromRtpRtcpConfiguration(
  ...
  result.fec_receive_statistics = configuration.fec_receive_statistics;
...
RTCPSender::RTCPSender(const Environment& env, Configuration config)
    : env_(env),
      ...
      fec_receive_statistics_(config.fec_receive_statistics),
  ...
  builders_[kRtcpFECr] = &RTCPSender::BuildFECR;
}
...
void RTCPSender::BuildFECR(const RtcpContext& ctx, PacketSender& sender) {
  rtcp::FecReceiverReport report;
  report.SetSenderSsrc(ssrc_);
  report.SetReportBlocks(CreateFECReportBlocks(ctx.feedback_state_));
  if (method_ == RtcpMode::kCompound || !report.report_blocks().empty()) {
    sender.AppendPacket(report);
  }
}
...
void RTCPSender::PrepareReport(const FeedbackState& feedback_state) {
  ...
  if (IsFlagPresent(kRtcpSr) || IsFlagPresent(kRtcpRr)) {
    ...
    if (IsFlagPresent(kRtcpRr) && !IsFlagPresent(kRtcpFECr)) {
      SetFlag(kRtcpFECr, true);
    }
    ...
    if (generate_report) {
      ...
      if (!sending_) SetFlag(kRtcpFECr, true);
    }
...
std::vector<rtcp::FECReportBlock> RTCPSender::CreateFECReportBlocks(
  const FeedbackState& feedback_state) {

  // std::cout<<"Run CreateFECReportBlocks"<<std::endl;

  std::vector<rtcp::FECReportBlock> result;
  if (!fec_receive_statistics_) return result;

  result = fec_receive_statistics_->RtcpReportBlocks(RTCP_MAX_REPORT_BLOCKS);

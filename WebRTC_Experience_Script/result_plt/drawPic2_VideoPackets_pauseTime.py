...
def plot_graph(path, save_to):
    ...
    framePauseTotal=data2['totalFreezesDuration'].to_list();
    framePause=[];
    for i in range(0,len(framePauseTotal)):
        if (i!=0):
            framePause.append(framePauseTotal[i]-framePauseTotal[i-1]);
        else:
            framePause.append(framePauseTotal[i]);
    framePause=[i*1e3 for i in framePause];

    # Get out packets
    data3 = pd.read_csv(os.path.join(path,'peer1/rtp_sender_egress.csv'));
    out_seq_num=data3['seq_num'].to_list();
    out_packet_type=data3['packet_type'].to_list();
    out_seq_type={};
    for now_num,now_type in zip(out_seq_num, out_packet_type):
        out_seq_type[now_num]=now_type;

    # Get in packets
    data4 = pd.read_csv(os.path.join(path,'peer2/rtp_video_stream_receiver2.csv'));
    print(data4.columns);
    in_timeline=data4['timestamp'].to_list();
    in_timeline = [(i - base_time) / 1e6 for i in in_timeline];
    in_seq_num=data4['seq_num'].to_list();

    # Get fec packets
    data5 = pd.read_csv(os.path.join(path,'peer2/forward_error_correction.csv'));
    print(data5.columns);
    sel_data5 = data5[data5['status']=='success'];
    fec_recover_seq_num_list=sel_data5['recover_seq_num'].to_list();
    fec_recover_seq_num = [];
    for seq_num_str in fec_recover_seq_num_list:
        for seq_num in str(seq_num_str).split("_"):
            fec_recover_seq_num.append(eval(seq_num));

    lost_video_packets=[];
    recover_video_packets=[];
    from_time=0;
    nowi=0;
    max_seq_num=0;
    for to_time in timeline2:
        now_lost_num=0;
        now_recover_num=0;
        while (nowi<len(in_timeline) and in_timeline[nowi]<=to_time):
            if (nowi==0 and max_seq_num==0):
                nowi+=1;
                max_seq_num=in_seq_num[nowi];
                continue;
            if (nowi!=0 and in_seq_num[nowi]<=max_seq_num):
                nowi+=1;
                continue;
            if (nowi!=0 and in_seq_num[nowi]-max_seq_num!=1):
                for it_seq_num in range(max_seq_num+1,in_seq_num[nowi]):
                    if (out_seq_type[it_seq_num]=='VideoPacket'):
                        if (it_seq_num in fec_recover_seq_num):
                            now_recover_num+=1;
                        else:
                            now_lost_num+=1;
            max_seq_num=max(max_seq_num,in_seq_num[nowi]);
            nowi+=1;
        lost_video_packets.append(now_lost_num);
        recover_video_packets.append(now_recover_num);
        from_time=to_time;
    ...
    ax1.fill_between(trace_timeline, trace_rate, color='darkblue', alpha=0.2, zorder=1, label='bandwidth (Kbps)')  # Fill the area under the bandwidth curve
    ...
    ax4.plot(timeline2, pixel, color='orange', linewidth=5, linestyle='-', alpha=1, label='Video resolution', zorder=5);
    ...
    for i in range(1,len(timeline2)):
        now_t.append(timeline2[i-1]);
        now_t.append(timeline2[i-1]);
        now_t.append(timeline2[i]);
        now_t2.append(timeline2[i - 1]+0.5);
        now_t2.append(timeline2[i - 1]+0.5);
        now_t2.append(timeline2[i]+0.5);
        lost_v_pkt.append(lost_video_packets[i-1]);
        lost_v_pkt.append(lost_video_packets[i]);
        lost_v_pkt.append(lost_video_packets[i]);
        recv_pkt.append(recover_video_packets[i-1]);
        recv_pkt.append(recover_video_packets[i]);
        recv_pkt.append(recover_video_packets[i]);
        p_time.append(framePause[i-1]);
        p_time.append(framePause[i]);
        p_time.append(framePause[i]);

    ax5.plot(now_t, lost_v_pkt, color='firebrick', linewidth=2, linestyle='-',label='Packets Lost', zorder=2);
    # ax5.fill_between(now_t, lost_v_pkt, color='darkred', alpha=0.5, linestyle='-', label='Packets Lost Number', zorder=15);
    ax5.plot(now_t, recv_pkt, color='darkgreen', linewidth=2, linestyle='-', label='Packets Recover', zorder=2);
    # ax5.fill_between(now_t, recv_pkt, color='darkgreen', alpha=0.5, linestyle='-', label='Packets Recover Number', zorder=15);

    ax2.plot(now_t, p_time, color='m', linewidth=2,linestyle='-',alpha=0.5, label='frame pause time (ms)');
    ...
    max_delay = max(max_delay, max(framePause));
    max_resoluton = max(pixel);
    max_pkt_num = max(max(lost_video_packets), max(recover_video_packets));

    ax1.set_ylim(0, max_rate * 1.4);
    ax2.set_ylim(0, max_delay * 1.4);
    ax4.set_ylim(0, max_resoluton * 1.5);
    ax5.set_ylim(0, max_pkt_num*1.5);

    ax2.set_ylabel('Delay (ms)',fontsize=30, fontweight='bold');
    ax5.set_ylabel('packets', fontsize=30, fontweight='bold');
    ax2.legend(loc='upper right', fontsize=25);
    ax4.legend(loc='upper center', fontsize=25);
    ax5.legend(loc='upper center', fontsize=25, bbox_to_anchor=(0.50, 0.93));
    ...
    ax4.spines['left'].set_position(('outward', -1295))
    ...
    if save_to:
        ...
        file_name = os.path.join(save_to, f'Pic2_VideoPackets_pauseTime.png');

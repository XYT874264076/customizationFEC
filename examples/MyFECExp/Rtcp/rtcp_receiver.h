...
class RTCPReceiver final {
 ...
 private:
  ...
  bool HandleFECReceiverReport(const rtcp::CommonHeader& rtcp_block,
                               PacketInformation* packet_information)
      RTC_EXCLUSIVE_LOCKS_REQUIRED(rtcp_receiver_lock_);
  
  void HandleFECReportBlock(const rtcp::FECReportBlock& report_block,
                            PacketInformation* packet_information,
                            uint32_t remote_ssrc)
      RTC_EXCLUSIVE_LOCKS_REQUIRED(rtcp_receiver_lock_);                  
  ...
  flat_map<uint32_t, ReportBlockData> received_report_blocks_
      RTC_GUARDED_BY(rtcp_receiver_lock_);
  flat_map<uint32_t, FECReportBlockData> received_fec_report_blocks_
      RTC_GUARDED_BY(rtcp_receiver_lock_);
  ...
  FECReportBlockDataObserver* const fec_report_block_data_observer_;
};
}  // namespace webrtc

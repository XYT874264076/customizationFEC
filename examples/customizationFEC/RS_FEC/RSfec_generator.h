#ifndef EXAMPLES_CUSTOMIZATION_RSFEC_RSFEC_GENERATOR_H_
#define EXAMPLES_CUSTOMIZATION_RSFEC_RSFEC_GENERATOR_H_
...
class RSfecGenerator : public VideoFecGenerator {
    friend class FlexfecSender;

  public:
    RSfecGenerator(const Environment& env,
                   int red_payload_type,
                   int rsfec_payload_type);
    ~RSfecGenerator();

    FecType GetFecType() const override {
        return VideoFecGenerator::FecType::kRSFecBlock;
    }
  ...
  private:
    ...
    RSfecGenerator(const Environment& env, std::unique_ptr<RSForwardErrorCorrection> fec);
...
# endif // EXAMPLES_CUSTOMIZATION_RSFEC_RSFEC_GENERATOR_H_
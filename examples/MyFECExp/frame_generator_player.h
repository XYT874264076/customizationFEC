...
#ifndef TEST_FRAME_GENERATOR_PLAYER_H_
#define TEST_FRAME_GENERATOR_PLAYER_H_
...
class FrameGeneratorPlayer : public MP4VideoPlayer {
 public:
  class SinkWantsObserver {
   public:
    // OnSinkWantsChanged is called when FrameGeneratorCapturer::AddOrUpdateSink is called.
    virtual void OnSinkWantsChanged(rtc::VideoSinkInterface<VideoFrame>* sink, const rtc::VideoSinkWants& wants) = 0;
   protected:
    virtual ~SinkWantsObserver() {}
  };
  
  FrameGeneratorPlayer(
    Clock* clock,
      std::unique_ptr<FrameGeneratorInterface> frame_generator,
      int target_fps,
      TaskQueueFactory& task_queue_factory);
  virtual ~FrameGeneratorPlayer();
...
#endif  // TEST_FRAME_GENERATOR_PLAYER_H_